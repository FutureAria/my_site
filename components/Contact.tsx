          ...
          <div id="contact-form" className="glass scroll-mt-24 rounded-2xl p-6 sm:p-8">
            ...
            {sent ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                {sendError && (
                  <div className="contact-error-panel rounded-xl border border-red-400/20 bg-red-500/10 p-4 text-sm text-red-100">
                    <p>{sendError}</p>
                    <a
                      href={`mailto:${data.email}?subject=${encodeURIComponent(`[문의] ${form.name || "포트폴리오"}`)}&body=${encodeURIComponent(form.message)}`}
                      className="contact-error-link mt-3 inline-flex max-w-full items-center gap-2 rounded-lg border border-red-300/20 bg-white/10 px-3 py-2 font-semibold text-white transition-colors hover:bg-white/15"
                    >
