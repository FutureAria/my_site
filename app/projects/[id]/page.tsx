...
> = {
  ...
  "KIS AI 트레이더": {
    headline: "개인 프로젝트 · 읽기 전용 · 주문 잠금",
    summary: "시장 데이터와 AI 판단 근거는 보여주고, 실제 주문 권한은 분리했습니다.",
    problem: "AI 판단과 주문 권한이 섞이면 방문자가 실거래로 오해할 수 있었습니다.",
    role: "수집, 판단 근거, 읽기 전용 화면을 직접 설계했습니다.",
    result: "공개 화면은 read-only, 실제 주문은 승인 전 잠금으로 분리했습니다.",
  },
  ...
  "AI 감정 분석 기반 음악 추천 서비스": {
    headline: "Claude 감정분석을 설명 가능한 추천으로",
    summary: "Claude 응답을 백엔드에서 정규화하고 감정 결과·추천 기준·히스토리를 분리해 설명 가능한 음악 추천으로 연결했습니다.",
    problem: "Claude 응답 형식이 흔들리면 추천과 화면이 함께 불안정해졌습니다.",
    role: "Spring Boot 백엔드 프록시와 Claude 응답 정규화를 맡았습니다.",
    result: "감정 기준으로 추천을 설명하고 사용자 기록으로 개인화하는 흐름을 운영 배포본에서 확인할 수 있게 했습니다.",
  },
  "ICPC MT 실시간 진행 시스템": {
    headline: "발표 화면과 폰 리모컨을 실시간 동기화",
    summary: "발표 화면이 상태를 broadcast하고 폰 리모컨이 구독해 슬라이드·음악·점수를 여러 기기에 실시간 동기화했습니다.",
    problem: "진행자가 노트북에 묶이고 점수·슬라이드가 기기마다 따로 놀았습니다.",
    role: "발표/리모컨/보기 화면 분리와 Flask-SocketIO broadcast를 구현했습니다.",
    result: "폰 하나로 슬라이드·배경음악·실시간 점수를 제어하며 진행할 수 있게 했습니다.",
  },
  MajorLink: {
    headline: "진행 중 · MVP 범위 먼저 검증",
    summary: "프로필, 모집글, 지원/승인을 1차 범위로 좁히고 제외 기능을 문서화했습니다.",
    problem: "아이디어만 있으면 매칭, 채팅, 추천 기능이 과하게 커질 수 있었습니다.",
    role: "MVP 포함 범위, 제외 범위, 일정표, 역할 분담표를 정리했습니다.",
    result: "다음 구현 단위를 프로필, 모집글, 지원/승인, 태그 검색으로 압축했습니다.",
  },
};
